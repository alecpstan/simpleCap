...
class InvestorsPage extends StatelessWidget {
  ...
  Future<void> _showSellSharesDialog(
    ...
    if (currentShares <= 0) {
      showInfoSnackbar(context, 'This investor has no shares to sell');
      return;
    }
...
class _InvestorCardState extends State<_InvestorCard> {
  ...
  void _showOptionGrantDetails(
    ...
    showDialog(
      context: context,
      builder: (ctx) => GrantDetailsDialog(
        grant: grant,
  ...
  Widget _buildTransactionItem(Transaction txn, CoreCapTableProvider provider) {
    final theme = Theme.of(context);
