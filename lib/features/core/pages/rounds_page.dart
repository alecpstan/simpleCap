...
class RoundsPage extends StatelessWidget {
  ...
  Widget _buildRoundsList(BuildContext context, CoreCapTableProvider provider) {
  ...
  Future<void> _showConvertNotesDialog(
    ...
    final outstanding = coreProvider.outstandingConvertibles;
    if (outstanding.isEmpty) {
      showInfoSnackbar(context, 'No outstanding convertibles to convert');
      return;
    }
...
class _ConvertNotesDialogState extends State<_ConvertNotesDialog> {
  ...
  Future<void> _convertSelected() async {
    int successCount = 0;
    final issuedBefore = widget.provider.getIssuedSharesBeforeRound(widget.round.id);
    ...
    for (final entry in _selections.entries) {
      ...
      try {
        await widget.convertiblesProvider.convertConvertible(
          convertibleId: entry.key,
...
class _RoundCardState extends State<_RoundCard> {
  ...
  Widget _buildInvestmentItem(Transaction t, CoreCapTableProvider provider) {
    final theme = Theme.of(context);
