...
class _VestingPageState extends State<VestingPage> {
  ...
  List<_VestingItem> _buildVestingItems(CoreCapTableProvider provider) {
  ...
  void _showAddMenu(BuildContext context, CoreCapTableProvider provider) {
  ...
  void _showAddTimeBasedVesting(
    ...
    if (transactionsWithoutVesting.isEmpty) {
      showInfoSnackbar(context, 'All investments already have vesting schedules');
      return;
    }
  ...
  void _showAddMilestone(BuildContext context, CoreCapTableProvider provider) {
    showDialog(
      context: context,
...
class _VestingScheduleDialogState extends State<VestingScheduleDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        const DialogCancelButton(),
        DialogPrimaryButton(onPressed: _save, label: isEditing ? 'Save' : 'Add'),
      ],
    );
  }
